...
const ClassSchedule = () => {
  ...
  return (
    <section id="schedule" className="relative z-20 py-24 bg-iron-gym-gym-charcoal">
      <div className="max-w-5xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            <h2 className="text-iron-gym-gym-accent font-bold tracking-widest uppercase mb-4 text-sm">Timetable</h2>
            <h3 className="text-4xl md:text-5xl font-black font-iron-gym-display uppercase">Class Schedule</h3>
          </div>
        </div>
        ...
        <div className="flex overflow-x-auto gap-2 mb-8 pb-4 hide-scrollbar">
          {days.map((day, idx) => (
            <button
              ...
              className={`min-w-[80px] py-3 rounded-xl font-bold uppercase tracking-wider text-sm transition-colors ${activeDay === idx ? 'bg-iron-gym-gym-accent text-iron-gym-gym-darker' : 'bg-iron-gym-gym-dark text-gray-400 hover:bg-white/10'}`}
            >
              {day}
            </button>
          ))}
        ...
        <div className="bg-iron-gym-gym-dark rounded-3xl overflow-hidden border border-white/5">
            ...
            {schedule.map((item, idx) => (
              <div key={idx} className="flex flex-col md:flex-row md:items-center justify-between p-6 hover:bg-white/5 transition-colors group cursor-pointer">
                <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-12 mb-4 md:mb-0">
                  <div className="font-iron-gym-display font-bold text-xl text-iron-gym-gym-accent w-24">
                    {item.time}
                  </div>
                  <div>
                ...
                <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto">
                  ...
                  <button className="px-6 py-2 rounded-full border border-white/20 text-sm font-bold uppercase tracking-wider group-hover:bg-white group-hover:text-iron-gym-gym-darker transition-colors">
                    Book
                  </button>
                </div>
              </div>
