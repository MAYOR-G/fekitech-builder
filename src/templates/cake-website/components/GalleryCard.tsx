...
export function GalleryCard({ item }: GalleryCardProps) {
  ...
  return (
    <article className="group cursor-pointer">
      <div className="relative overflow-hidden bg-sweet-cake-chantilly h-full w-full aspect-[4/3]">
        <ImageFrame
          src={item.image}
          alt={item.title}
        ...
        >
          ...
          <div className="absolute inset-0 bg-sweet-cake-ganache/30 opacity-0 group-hover:opacity-100 transition-opacity duration-700 ease-in-out" />
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-8 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-700 ease-out z-10">
            ...
            <h3 className="font-sweet-cake-display text-2xl font-normal leading-tight text-white">
              {item.title}
            </h3>
