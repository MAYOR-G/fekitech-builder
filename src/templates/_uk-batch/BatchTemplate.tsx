...
function Hero({ layout, content, navigate }: { layout: number; content: BatchData; navigate: (target: string, e?: MouseEvent<HTMLElement>) => void }) {
  const renderCopy = () => <div className="ukb-hero-copy"><p className="ukb-eyebrow" {...text("hero.eyebrow")}>{content.hero.eyebrow}</p><h1 {...text("hero.title")}>{content.hero.title}</h1><p {...text("hero.text")}>{content.hero.text}</p><div className="ukb-actions"><a href={`#${content.hero.primaryPage}`} onClick={(e) => navigate(content.hero.primaryPage, e)} {...link("hero.primaryLabel", "hero.primaryPage")}>{content.hero.primaryLabel}</a><a href={`#${content.hero.secondaryPage}`} onClick={(e) => navigate(content.hero.secondaryPage, e)} {...link("hero.secondaryLabel", "hero.secondaryPage")}>{content.hero.secondaryLabel}</a></div><small {...text("hero.note")}>{content.hero.note}</small></div>;
  const renderMainImage = (className: string = "ukb-hero-main") => <img className={className} src={content.hero.image} alt={content.hero.imageAlt} {...image("hero.image", "hero.imageAlt")} />;
  const renderSecondImage = (className: string = "ukb-hero-secondary") => <img className={className} src={content.hero.imageSecondary} alt={content.hero.imageSecondaryAlt} {...image("hero.imageSecondary", "hero.imageSecondaryAlt")} />;
  if (layout === 1) return <section className="ukb-hero ukb-hero-academy"><aside><span>EST.</span><b>1898</b><i>York</i></aside>{renderCopy()}<div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}<strong>Prospectus<br />2026–27</strong></div></section>;
  if (layout === 2) return <section className="ukb-hero ukb-hero-hotel">{renderMainImage("ukb-hero-backdrop")}<div className="ukb-hotel-panel">{renderCopy()}{renderSecondImage()}<span>16 rooms · one lovely house</span></div></section>;
  if (layout === 3) return <section className="ukb-hero ukb-hero-legal"><div className="ukb-legal-mark"><span>MB</span><small>01 / Counsel</small></div>{renderCopy()}<div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}</div></section>;
  if (layout === 4) return <section className="ukb-hero ukb-hero-ledger"><div className="ukb-ledger-grid"><span>01</span><span>Plan</span><span>Measure</span><span>Move</span></div>{renderCopy()}<div className="ukb-hero-media">{renderMainImage()}<div><b>98%</b><small>client retention</small></div></div></section>;
  if (layout === 5) return <section className="ukb-hero ukb-hero-events">{renderSecondImage()}{renderCopy()}{renderMainImage()}<em>Celebrations, composed beautifully</em></section>;
  if (layout === 6) return <section className="ukb-hero ukb-hero-vet"><div className="ukb-vet-orbit">{renderMainImage()}{renderSecondImage()}<span>Open today</span></div>{renderCopy()}<aside><b>Need advice?</b><small>Call our Bristol team</small></aside></section>;
  if (layout === 7) return <section className="ukb-hero ukb-hero-florist"><div className="ukb-florist-word">FIELD<br /><i>&amp;</i> STEM</div><div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}</div>{renderCopy()}</section>;
  if (layout === 8) return <section className="ukb-hero ukb-hero-nursery"><div className="ukb-play-shape">A<br />B<br />C</div>{renderCopy()}<div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}<span>Learn · play · belong</span></div></section>;
  if (layout === 9) return <section className="ukb-hero ukb-hero-motor"><div className="ukb-motor-code">BMW / 09</div>{renderCopy()}<div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}<span>MOT • SERVICE • REPAIR</span></div></section>;
  if (layout === 10) return <section className="ukb-hero ukb-hero-garden">{renderMainImage("ukb-hero-backdrop")}<div className="ukb-garden-caption"><span>Landscape notes № 10</span>{renderCopy()}</div>{renderSecondImage()}</section>;
  if (layout === 11) return <section className="ukb-hero ukb-hero-restaurant"><div className="ukb-restaurant-date">SEASON / 08</div><div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}</div>{renderCopy()}<div className="ukb-restaurant-menu"><span>Tonight</span><b>Market menu</b><small>Six courses · £92</small></div></section>;
  if (layout === 12) return <section className="ukb-hero ukb-hero-pub"><div className="ukb-pub-sign">N<span>&amp;</span>C</div>{renderCopy()}<div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}</div><div className="ukb-pub-taps">CASK / KITCHEN / MUSIC</div></section>;
  if (layout === 13) return <section className="ukb-hero ukb-hero-burger"><div className="ukb-burger-stamp">SMASHED<br />TO ORDER</div>{renderCopy()}<div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}</div><div className="ukb-burger-price">FROM £9.50</div></section>;
  return <section className="ukb-hero ukb-hero-pizza"><div className="ukb-pizza-word">LUMA</div>{renderCopy()}<div className="ukb-hero-media">{renderMainImage()}{renderSecondImage()}</div><aside><b>48h</b><span>slow-fermented dough</span></aside></section>;
}
