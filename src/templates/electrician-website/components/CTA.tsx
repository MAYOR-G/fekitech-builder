    ...
    <section id="contact" className="px-5 py-20 lg:px-8">
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[1.4rem] bg-circuit-hero text-white shadow-deep">
        <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[0.92fr_1.08fr] lg:p-10">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-spark-electric-safety">
              Book service
            </p>
            <h2 className="mt-4 font-spark-electric-display text-4xl font-extrabold leading-[1.02] tracking-[-0.05em] sm:text-5xl">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {contactCards.map((card) => {
              ...
              return (
                <div
                  key={card.title}
                  className="rounded-[1rem] border border-white/12 bg-white/[0.08] p-5 shadow-insetLine transition duration-300 hover:-translate-y-1 hover:border-spark-electric-cyan/32"
                >
                  <Icon className="h-5 w-5 text-spark-electric-safety" aria-hidden="true" />
                  <p className="mt-4 text-xs font-black uppercase tracking-[0.16em] text-spark-electric-cyan">
                    {card.title}
                  </p>
